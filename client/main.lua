...
local function OpenStable()
    ...
    TriggerServerEvent("lxr-stable:AskForMyHorses")
end
...
local function SetHorseName(data)
	...
	CreateThread(function()
		...
		if (GetOnscreenKeyboardResult()) then
			...
			TriggerServerEvent('lxr-stable:BuyHorse', data, HorseName)
			Wait(1500)
			SendNUIMessage(
				{
					action = "show",
...
local function CloseStable()
	...
	if DadosEncoded ~= "{}" then
		TriggerServerEvent("lxr-stable:UpdateHorseComponents", dados, IdMyHorse, MyHorse_entity )
	end
end
...
local function WhistleHorse()
    if SpawnplayerHorse ~= 0 then
        ...
        TriggerServerEvent('lxr-stable:CheckSelectedHorse')
        Wait(100)
        InitiateHorse()
    end
end
...
RegisterNUICallback("selectHorse", function(data)
	TriggerServerEvent("lxr-stable:SelectHorseWithId", tonumber(data.horseID))
end)

RegisterNUICallback("sellHorse", function(data)
	DeleteEntity(showroomHorse_entity)
	TriggerServerEvent("lxr-stable:SellHorseWithId", tonumber(data.horseID))
	TriggerServerEvent("lxr-stable:AskForMyHorses")
...
RegisterNetEvent('lxr-stable:client:UpdadeHorseComponents', function(horseEntity, components)
    for _, value in pairs(components) do
        NativeSetPedComponentEnabled(horseEntity, value)
    end
end)

RegisterNetEvent("lxr-stable:ReceiveHorsesData", function(dataHorses)
	SendNUIMessage(
		{
